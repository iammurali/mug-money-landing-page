    ...
    estreeWalker.walk(root, {
        enter(node, parent) {
            ...
            if (node.type === 'Identifier') {
                ...
                node.params.forEach((p) => estreeWalker.walk(p.type === 'AssignmentPattern' ? p.left : p, {
                    enter(child, parent) {
...
function transformRequest(url, server, options = {}) {
    ...
    if (pending) {
        debugTransform(`[reuse pending] for ${prettifyUrl(url, server.config.root)}`);
        return pending;
    }
...
async function resolveConfig(inlineConfig, command, defaultMode = 'development') {
    ...
    if (((_j = config.build) === null || _j === void 0 ? void 0 : _j.terserOptions) && config.build.minify === 'esbuild') {
        logger.warn(chalk__default.yellow(`build.terserOptions is specified but build.minify is not set to use Terser. ` +
...
//# sourceMappingURL=dep-af7b2554.js.map
